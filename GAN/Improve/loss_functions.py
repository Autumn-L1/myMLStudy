...
class Label_smooth_GANLoss:
    ...
    def discriminator_loss(self, real_output, fake_output):
        # 真实样本的目标标签为0.9
        real_labels = torch.full_like(real_output, 0.9, device=self.device)
        # 生成样本的目标标签为0.1
        fake_labels = torch.full_like(fake_output, 0.1, device=self.device)
    ...
    def generator_loss(self, fake_output):
    # 生成器希望判别器将生成样本判断为真实样本
        target_labels = torch.full_like(fake_output, 0.9, device=self.device)
        g_loss = self.criterion(fake_output, target_labels)
        return g_loss